...
public class CharacterAnimationController : MonoBehaviour
{
    ...
    public void Hit()
    {
        ...
        if ((m_CharacterParamManager.CharacterType == CharacterParam.GameCharacterType.Healer) && (m_CharacterParamManager.ButtonNo == 3))
        {
            ...
            var minHp = m_CharacterParamManager(0);
            m_CharacterParamManager = CharacterParams(0);
            for (int i = 0; i < 3; i++)
                {
                m_CharacterParamManager = m_CharacterParamManager;
            }

        }

        GameMainManager s_GameMainManager = null;
        ...
        {
            var pos = (Random.Range(0.0f, 2.0f));


            t_CharacterParammanager = GameMainManager.CharacterParamManagers[pos];
        }
    }
